...
from utils.albu import IsotropicResize
...
class SBI_Dataset(Dataset):
    def __init__(self, image_path, phase='train', image_size=224, 
        ...
        self.dfdc = self.create_train_transforms(size=self.image_size)
    ...
    def get_source_transforms(self, size=224):
        return self.create_train_transforms(size=size)
    ...
    def self_blending(self,img,landmark, bbox):
        ...
        (img_f,_,__,___,y0_new,
            y1_new,x0_new,x1_new)=self.crop_face(img,landmark,bbox,margin=False,
                        crop_by_bbox=True,abs_coord=True,phase=self.phase)
    ...
    def dynamic_blend(self, source,target,mask):
        ...
        img_blended=(mask_blured * source + (1 - mask_blured) * target)
        inner = mask_blured * source 
        outer = (1 - mask_blured) * target
