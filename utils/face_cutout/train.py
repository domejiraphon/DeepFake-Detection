...
def train(name, df):
    ...
    val_data_loader = DataLoader(data_val, batch_size=config.valid_batch_size,
    ...
    for epoch in range(config.epochs):
        ...
        train_metrics = train_epoch(model, train_data_loader, optimizer,
                                    criterion, epoch)
        ...
        print(
            f"TRAIN_AUC = {train_metrics['train_auc']}, TRAIN_LOSS = {train_metrics['train_loss']}")
        print(
            f"VALID_AUC = {valid_metrics['valid_auc']}, VALID_LOSS = {valid_metrics['valid_loss']}")
...
def train_epoch(model, train_data_loader, optimizer, criterion, epoch):
    ...
    for batch in tqdm(train_data_loader):
        ...
        if idx % 1000 == 0:
            with torch.no_grad():
                ...
                train_f1_05 = metrics.f1_score(temp_t,
                                               (temp_correct_preds >= 0.5) * 1)
                ...
                train_ap = metrics.average_precision_score(temp_t,
                                                           temp_correct_preds)
    ...
    with torch.no_grad():
        ...
        train_f1_05 = metrics.f1_score(targets,
                                       (correct_predictions >= 0.5) * 1)
        train_acc_05 = metrics.accuracy_score(targets,
                                              (correct_predictions >= 0.5) * 1)
        train_balanced_acc_05 = metrics.balanced_accuracy_score(
            targets, (correct_predictions >= 0.5) * 1
        )
        train_ap = metrics.average_precision_score(targets,
                                                   correct_predictions)
        train_log_loss = metrics.log_loss(
...
def valid_epoch(model, val_data_loader, criterion, epoch):
    ...
    valid_acc_05 = metrics.accuracy_score(targets,
                                          (correct_predictions >= 0.5) * 1)
    ...
    valid_log_loss = metrics.log_loss(targets,
                                      expand_prediction(correct_predictions))
...
def expand_prediction(arr):
    ...
    return np.clip(np.concatenate((1.0 - arr_reshaped, arr_reshaped), axis=1),
                   0.0, 1.0)
