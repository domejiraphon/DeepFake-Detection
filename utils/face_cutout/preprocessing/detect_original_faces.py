...
def process_videos(videos, root_dir):
    ...
    for item in tqdm(loader):
        ...
        if (video == None or indices == None or frames == None):
            continue
        batches = [frames[i:i + detector._batch_size] for i in
                   range(0, len(frames), detector._batch_size)]
        for j, frames in enumerate(batches):
            result.update(
                {
                    int(j * detector._batch_size) + i: b for i, b in
                    zip(indices, detector._detect_faces(frames))
                }
            )
...
def main():
    ...
    for path in originals:
        ...
        if (
        os.path.exists(os.path.join(ROOT_DIR, 'boxes', video_id + ".json"))):
            continue
        else:
            _.append(path)
    originals = _
