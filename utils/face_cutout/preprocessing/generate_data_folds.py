...
def main(n_splits):
    ...
    for fold in range(n_splits):
        folds.append(list(
            range(sz * fold, sz * fold + sz if fold < n_splits - 1 else 50)))
    print(folds)
        ...
        if "dfdc" in d:
            ...
            for f in os.listdir(os.path.join(ROOT_DIR, d)):
                if "metadata.json" in f:
                    with open(os.path.join(ROOT_DIR, d,
    ...
    for img_path, label, ori_vid in tqdm(data):
        ...
        file = path.name
        assert video_fold[video] == video_fold[
            ori_vid], "original video and fake have leak  {} {}".format(
            ori_vid,
            video)
    ...
    pd.DataFrame(fold_data,
                 columns=["video", "file", "label", "original", "frame",
                          "fold"]).to_csv('full_folds.csv', index=False)
