...
def get_video_paths(root_dir):
    ...
    for json_path in tqdm(glob(os.path.join(root_dir, "*/metadata.json"))):
        ...
        for k, v in metadata.items():
            ...
            bboxes_path = os.path.join(root_dir, "boxes",
                                       original[:-4] + ".json")
...
def extract_video(param, root_dir, crops_dir):
    ...
    for i in range(frames_num):
        ...
        if not success or str(i) not in bboxes_dict or bboxes_dict[
            str(i)] is None:
            continue
...
def main():
    ...
    for video_path, _ in tqdm(params):
        ...
        if (os.path.exists(
                os.path.join(ROOT_DIR, 'face_crops', video_id, 'done.txt'))):
            continue
        else:
    ...
    with Pool(processes=cpu_count() - 4) as p:
        with tqdm(total=len(params)) as pbar:
            for v in p.imap_unordered(partial(extract_video, root_dir=ROOT_DIR,
                                              crops_dir=CROPS_DIR), params):
                pbar.update()
