...
mtcnn_detector = MTCNN(margin=0, thresholds=[0.65, 0.75, 0.75], device="cuda")
dlib_detector = dlib.get_frontal_face_detector()
dlib_predictor = dlib.shape_predictor(
    "./utils/face_cutout/libs/shape_predictor_68_face_landmarks.dat")
...
def face_cutout(
    ...
    if choice == 0:
        image = sensory_cutout(image, original, landmarks, mask, probability,
                               cutout_fill, threshold)
    elif choice == 1:
        image = convex_hull_cutout(image, original, mask, probability,
                                   cutout_fill, threshold)
...
def sensory_cutout(
    ...
    if landmarks is None:
        ...
        batch_boxes, conf, landmarks = mtcnn_detector.detect(frame_img,
                                                             landmarks=True)
...
def remove_eyes(image, landmarks, mask=None, cutout_fill=0, threshold=0.3):
    ...
    line = cv2.line(
        np.zeros_like(image[..., 0]), (x1, y1), (x2, y2), color=(1),
        thickness=2
    )
...
def remove_nose(image, landmarks, mask=None, cutout_fill=0, threshold=0.3):
    ...
    y4 = int((y1 + y2) / 2)
    line = cv2.line(
        np.zeros_like(image[..., 0]), (x3, y3), (x4, y4), color=(1),
        thickness=2
    )
    w = _distance((x1, y1), (x2, y2))
...
def _remove(image, mask, w, line, cutout_fill, threshold):
    ...
    for i in range(3,
                   7):  # Try multiple times to get max overlap below threshold
...
def convex_hull_cutout(
    ...
    if polygon is not None:
        if cutout_fill == -1:
            image_[polygon > 0] = np.random.randint(0, 255,
                                                    image_[polygon > 0].shape)
        else:
