...
class FaceDataset(Dataset):
    def __init__(
        ...
        print(
            "real {} fakes {} mode {}".format(
                len(rows[rows["label"] == 0]), len(rows[rows["label"] == 1]),
                self.mode
            )
        )
    ...
    def __getitem__(self, index: int):
        ...
        while (True):
            video, file, label, original, frame, fold, dataset = self.data[
                index]
            ...
            # Load image and mask
            img_path = os.path.join(ROOT_DIR, dataset, "face_crops", video,
                                    file)
            ori_path = os.path.join(ROOT_DIR, dataset, "face_crops", original,
                                    file)
            ...
            if label == 1:
                mask_path = os.path.join(ROOT_DIR, dataset, "diffs", video,
                                         file[:-4] + "_diff.png")
                if os.path.exists(mask_path):
                    mask_image = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
                else:
                    d, a = compare_ssim(ori_image, image, multichannel=True,
                                        full=True)
                    a = 1 - a
            ...
            if (self.mode == "train" and self.cutout):
                landmark_path = os.path.join(ROOT_DIR, dataset,
                ...
                try:
                    ...
                    image = face_cutout(image, ori_image, landmarks,
                                        mask_image,
                                        cutout_fill=self.cutout_fill)
            ...
            transNormalize = Normalize(mean=self.normalize['mean'],
    ...
    def _oversample(self, rows, seed):
        ...
        if self.mode == "train" and num_real > num_fakes:
            fakes = fakes.sample(n=int(num_real), replace=True,
                                 random_state=seed)
