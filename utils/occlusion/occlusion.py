...
sys.path.append("./utils/occlusion")

from occlusion_utils.utils import *
from occlusion_utils.colour_transfer import *
from occlusion_utils.paste_over import *
from occlusion_utils.random_shape_generator import *
...
class Occlusion_Generator:
    def __init__(self, occluder_img_dataset, occluder_mask_dataset, img_shape):
        self.cfg, self.args = Occlusion_Generator.setup(occluder_img_dataset,
                                                        occluder_mask_dataset)
        self.img_shape = img_shape
        self.image_augmentor = get_src_augmentor()
        self.occluder_augmentor = get_occluder_augmentor()
        self.occluder_mask_list = self.preprocess(occluder_mask_dataset)
        self.occluder_img_list = self.preprocess(occluder_img_dataset)

        logger.info("Read occlusion dataset...")

    def occlude_images(self, src_img, src_mask, src_img2=None):

        # src_img = cv2.cvtColor((src_img*255).astype(np.uint8), cv2.COLOR_RGB2BGR)
        # src_mask = cv2.cvtColor((src_mask*255).astype(np.uint8), cv2.COLOR_RGB2BGR)
        # src_img = (src_img*255).astype(np.uint8)
        src_img = cv2.resize(src_img, (self.img_shape, self.img_shape),
                             interpolation=cv2.INTER_LANCZOS4)
        src_mask = (src_mask * 255).astype(np.uint8)
        src_mask = cv2.resize(src_mask, (self.img_shape, self.img_shape),
                              interpolation=cv2.INTER_LANCZOS4)
       
        if np.allclose(src_mask, np.zeros_like(src_mask)):
            return cv2.cvtColor(src_img, cv2.COLOR_BGR2RGB), src_mask
        # src_mask=cv2.cvtColor(src_mask,cv2.COLOR_RGB2GRAY)
        idx = np.random.randint(0, len(self.occluder_img_list))
        occluder_img_path = self.occluder_img_list[idx]
        occluder_mask_path = self.occluder_mask_list[idx]

        # get occluder img and mask
        if self.args["randomOcclusion"]:
            occluder_img, occluder_mask = get_randomOccluderNmask()
        else:
            occluder_img, occluder_mask = get_occluderNmask2(occluder_img_path,
                                                             occluder_mask_path,
                                                             self.img_shape)
        if (occluder_img is None) or (occluder_mask is None):
            return None, None 
        ...
        # augment occluders
        occluder_img, occluder_mask = augment_occluder(self.occluder_augmentor,
                                                       occluder_img,
                                                       occluder_mask, src_rect)
        ...
        if self.args["rotate_around_center"]:
            src_center = (
            src_rect[0] + (src_rect[2] / 2), (src_rect[1] + src_rect[3] / 2))
            rotation = angle3pt((src_center[0], occluder_coord[1]), src_center,
                                occluder_coord)
        ...
        result_img, result_mask, occlusion_mask = paste_over(occluder_img,
                                                             occluder_mask,
                                                             src_img, src_mask,
                                                             occluder_coord,
                                                             occlusion_mask,
        ...
        def convert(result_img, result_mask, occlusion_mask):
            kernel = np.ones((5,5),np.uint8)
            occlusion_mask_edges = cv2.dilate(occlusion_mask, kernel,
                            iterations=2) - cv2.erode(occlusion_mask, kernel, iterations=2)
            ...
            result_img = np.where(
                np.dstack((np.invert(filtered_occlusion_mask_edges == 255),) * 3),
                result_img, blurred_image)

            # augment occluded image
            """
            transformed = self.image_augmentor(image=result_img, mask=result_mask,
        ...
        result_img, result_mask = convert(result_img, result_mask, occlusion_mask)
       
        if src_img2 is not None:
            src_img2 = cv2.resize(src_img2, (self.img_shape, self.img_shape),
                             interpolation=cv2.INTER_LANCZOS4)
            result_img2, result_mask2, occlusion_mask2 = paste_over(occluder_img,
                                                             occluder_mask,
                                                             src_img2, src_mask,
                                                             occluder_coord,
                                                             occlusion_mask,
                                                             self.args[
                                                                 "randomOcclusion"])
            result_img2, result_mask2 = convert(result_img2, result_mask, occlusion_mask)
           
            return result_img, result_mask, result_img2, result_mask2
        else:
            return result_img, result_mask
    ...
    @staticmethod
    def setup(occluder_img_dataset, occluder_mask_dataset):

        config = "utils/occlusion/configs/baseline.yaml"
    ...
    def preprocess(self, path):
        ...
        all_path = []
        for p in path:
            all_path += sorted(glob.glob(os.path.join(p, '*')))
        return all_path
