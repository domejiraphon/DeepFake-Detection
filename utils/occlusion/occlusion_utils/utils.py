...
def validate_path(name, dir):
    if not os.path.exists(dir) or not os.path.isdir(dir):
        raise ValueError(
            f'The path for the directory "{dir}"" does not exist or is not a folder')
...
def resize_by_factor(im, factor):
    ...
    return cv2.resize(im, new_size, fx=factor, fy=factor,
...
def augment_occluder(aug, occluder_img, occluder_mask, src_rect):
    ...
    # random resize
    try:
        scale_factor = (((src_rect[2] * src_rect[3])) / (
                    occluder_rect[2] * occluder_rect[3])) * np.random.uniform(
            0.5, 1)
...
def get_randomOccluderNmask():
    ...
    texture_img = \
    random.sample(glob.glob(f"./dataset/DTD/images/{texture_choice}/*.jpg"),
                  1)[0]
    ...
    try:
        occluder_img = cv2.bitwise_and(ori_occluder_img, ori_occluder_img,
                                       mask=occluder_mask)
    except Exception as e:
        print(e)
        return
    occluder_rect = cv2.boundingRect(occluder_mask)
    cropped_occluder_mask = occluder_mask[occluder_rect[1]:(
                occluder_rect[1] + occluder_rect[3]), occluder_rect[0]:(
                occluder_rect[0] + occluder_rect[2])]
    cropped_occluder_img = occluder_img[occluder_rect[1]:(
                occluder_rect[1] + occluder_rect[3]), occluder_rect[0]:(
                occluder_rect[0] + occluder_rect[2])]
...
def get_srcNmask(image_file, img_path, mask_path):
    ...
    src_img = cv2.imread(os.path.abspath(os.path.join(img_path, image_file)),
                         -1)
    src_img = cv2.cvtColor(src_img, cv2.COLOR_BGR2RGB)

    src_mask = cv2.imread(mask_path + f"{img_name}.png")
    src_mask = cv2.resize(src_mask, (1024, 1024),
                          interpolation=cv2.INTER_LANCZOS4)
...
def get_occluderNmask2(img_path, mask_path, img_shape):
    ori_occluder_img = cv2.resize(cv2.imread(img_path), (img_shape, img_shape),
                                  interpolation=cv2.INTER_LANCZOS4)

    occluder_mask = cv2.imread(mask_path)
    occluder_mask = cv2.cvtColor(occluder_mask, cv2.COLOR_BGR2GRAY)

    occluder_mask = cv2.resize(occluder_mask, (
...
def get_occluderNmask(occluder_file, img_path, mask_path):
    ...
    ori_occluder_img = cv2.imread(
        os.path.abspath(os.path.join(img_path, occluder_file)), -1)
    try:
        ori_occluder_img = cv2.cvtColor(ori_occluder_img, cv2.COLOR_BGR2RGB)
    except Exception as e:
        print(e)
    ...
    occluder_mask = cv2.imread(
        os.path.abspath(os.path.join(mask_path, occluder_name + ".png")))
    occluder_mask = cv2.cvtColor(occluder_mask, cv2.COLOR_BGR2GRAY)
