...
class RetinaFace(pl.LightningModule):  # pylint: disable=R0901
    def __init__(self, config: Adict[str, Any]) -> None:
        ...
        self.prior_box = object_from_dict(self.config.prior_box,
    ...
    def forward(self, batch: torch.Tensor) -> Tuple[
        torch.Tensor, torch.Tensor, torch.Tensor]:  # type: ignore
    ...
    def configure_optimizers(
            self,
    ) -> Tuple[Callable[[bool], Union[
        Optimizer, List[Optimizer], List[LightningOptimizer]]], List[Any]]:
        optimizer = object_from_dict(
            self.config.optimizer,
            params=[x for x in self.model.parameters() if x.requires_grad]
        )

        scheduler = object_from_dict(self.config.scheduler,
                                     optimizer=optimizer)
    ...
    def training_step(self, batch: Dict[str, torch.Tensor],
        ...
        loss_localization, loss_classification, loss_landmarks = self.loss(out,
                                                                           targets)
        ...
        self.log("train_classification", loss_classification, on_step=True,
                 on_epoch=True, logger=True, prog_bar=True)
        self.log("train_localization", loss_localization, on_step=True,
                 on_epoch=True, logger=True, prog_bar=True)
        self.log("train_landmarks", loss_landmarks, on_step=True,
                 on_epoch=True, logger=True, prog_bar=True)
        self.log("train_loss", total_loss, on_step=True, on_epoch=True,
                 logger=True, prog_bar=True)
        self.log("lr", self._get_current_lr(), on_step=True, on_epoch=True,
                 logger=True, prog_bar=True)
    ...
    def validation_step(self, batch: Dict[str, torch.Tensor],
        ...
        for batch_id in range(batch_size):
            boxes = decode(
                location.data[batch_id], self.prior_box.to(images.device),
    ...
    def validation_epoch_end(self, outputs: List) -> None:
        ...
        _, _, average_precision = recall_precision(result_gt,
                                                   result_predictions, 0.5)

        self.log("epoch", self.trainer.current_epoch, on_step=False,
                 on_epoch=True, logger=True)  # type: ignore
        self.log("val_loss", average_precision, on_step=False, on_epoch=True,
                 logger=True)

    def _get_current_lr(self) -> torch.Tensor:  # type: ignore
        lr = [x["lr"] for x in self.optimizers[0].param_groups][
            0]  # type: ignore
...
def main() -> None:
    ...
    Path(config.checkpoint_callback.filepath).mkdir(exist_ok=True,
                                                    parents=True)
