...
def get_args() -> Any:
    ...
    arg("-i", "--input_path", type=Path, help="Path with images.",
        required=True)
    arg("-c", "--config_path", type=Path, help="Path to config.",
        required=True)
    arg("-o", "--output_path", type=Path, help="Path to save jsons.",
        required=True)
    arg("-v", "--visualize", action="store_true", help="Visualize predictions")
    arg("-m", "--max_size", type=int,
        help="Resize the largest side to this number", default=960)
    arg("-b", "--batch_size", type=int, help="batch_size", default=1)
    arg("-j", "--num_workers", type=int, help="num_workers", default=12)
    arg("--confidence_threshold", default=0.7, type=float,
    ...
    arg("--folder_in_name", action="store_true",
        help="Add folder to the saved labels.")
    return parser.parse_args()


class InferenceDataset(Dataset):
    def __init__(
            self, file_paths: List[Path], max_size: int,
...
def unnormalize(image: Union[np.ndarray, torch.Tensor]) -> Union[
...
def process_predictions(
    ...
    scale1 = torch.from_numpy(np.tile([image_width, image_height], 5)).to(
        loc.device)
    ...
    for batch_id in range(batch_size):
        ...
        unpadded = unpad_from_size(pads_numpy, bboxes=boxes,
                                   keypoints=landmarks)

        resize_coeff = max(original_shapes[batch_id]) / max(image_height,
                                                            image_width)

        boxes = (unpadded["bboxes"] * resize_coeff).astype(int)
        landmarks = (unpadded["keypoints"].reshape(-1,
                                                   10) * resize_coeff).astype(
            int)
...
def main() -> None:
    ...
    hparams.update(
        {
            ...
            "prior_box": object_from_dict(hparams["prior_box"],
    ...
    dataset = InferenceDataset(file_paths, max_size=args.max_size,
...
def predict(dataloader: torch.utils.data.DataLoader, model: nn.Module,
            hparams: dict, device: torch.device) -> None:
    model.eval()
