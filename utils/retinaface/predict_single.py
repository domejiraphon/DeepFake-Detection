    ...
    def predict_jsons(
        ...
        with torch.no_grad():
            ...
            scale_landmarks = torch.from_numpy(
                np.tile(transformed_size, 5)).to(self.device)
            ...
            loc, conf, land = self.model(
                torched_image.unsqueeze(0))  # pylint: disable=E1102
            ...
            for box_id, bbox in enumerate(boxes_np):
                ...
                annotations += [
                    {
                        "bbox": np.round(bbox.astype(float),
                                         ROUNDING_DIGITS).tolist(),
                        "score": np.round(scores, ROUNDING_DIGITS)[box_id],
                        "landmarks": np.round(
                            landmarks_np[box_id].astype(float),
                            ROUNDING_DIGITS)
                            .reshape(-1, 2)
                            .tolist(),
                    }
