...
def random_crop(
    ...
    for _ in range(250):
        ...
        mask_a = np.logical_and(roi[:2] < centers, centers < roi[2:]).all(
            axis=1)
...
class Preproc:
    ...
    def __call__(self, image: np.ndarray, targets: np.ndarray) -> Tuple[
        ...
        image_t, boxes_t, landmarks_t = random_horizontal_flip(image_t,
                                                               boxes_t,
                                                               landmarks_t)
