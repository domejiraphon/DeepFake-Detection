...
class MultiBoxLoss(nn.Module):
    ...
    def forward(
            self, predictions: Tuple[torch.Tensor, torch.Tensor, torch.Tensor],
        ...
        landmarks_t = torch.zeros(num_predicted_boxes, num_priors, 10).to(
            targets[0].device)
        conf_t = torch.zeros(num_predicted_boxes, num_priors).to(
            targets[0].device).long()
        ...
        loss_landm = F.smooth_l1_loss(landmarks_p, landmarks_t,
                                      reduction="sum")
        ...
        conf_p = confidence_data[(pos_idx + neg_idx).gt(0)].view(-1,
                                                                 self.num_classes)
