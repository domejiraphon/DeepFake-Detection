...
class FaceDetectionDataset(data.Dataset):
    def __init__(
        ...
        self.labels = [x for x in labels if
                       (image_path / x["file_name"]).exists()]
...
def random_rotate_90(image: np.ndarray, annotations: np.ndarray) -> Tuple[
    ...
    transformed = transform(
        image=image, keypoints=keypoints.tolist(), bboxes=boxes.tolist(),
...
def detection_collate(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {"image": torch.stack(images), "annotation": annotation,
