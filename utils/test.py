...
parser.add_argument('--dataset_path', type=str, default="/vast/jy3694/captcha_dataset")
...
def main():
    ...
    with torch.no_grad():
        ...
        for challenge_path in sorted(glob.glob(os.path.join(args.dataset_path, "*")))[1:]:
            ...
            for person in sorted(glob.glob(os.path.join(challenge_path, "*"))):
                ...
                print(f"chal_name: {chal_name} person: {name}")
                ...
                if len(all_path) == 1:
                    real_path = os.path.join(args.real_path, 
                                    all_path[0].split('/')[4],
                                    args.dataset_path.split('/')[3].split('_')[0],
                                    "real")
                ...
                if args.plot_graph:
                    ...
                    plt.plot(new_time, fake_scores, 'r')
        ...
        df.to_csv(os.path.join(args.dataset_path, 'all_anomaly_test.csv'))
