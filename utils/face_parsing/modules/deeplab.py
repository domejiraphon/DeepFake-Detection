...
class DeeplabV3(nn.Module):
    def __init__(self,
        ...
        self.map_convs = nn.ModuleList([
            ...
            nn.Conv2d(in_channels, hidden_channels, 3, bias=False,
        ...
        self.global_pooling_conv = nn.Conv2d(in_channels, hidden_channels, 1,
                                             bias=False)
    ...
    def _global_pooling(self, x):
        if self.training or self.pooling_size is None:
            ...
            padding = (
                (pooling_size[1] - 1) // 2,
                (pooling_size[1] - 1) // 2 if pooling_size[1] % 2 == 1 else (
                                                                                        pooling_size[
                                                                                            1] - 1) // 2 + 1,
                (pooling_size[0] - 1) // 2,
                (pooling_size[0] - 1) // 2 if pooling_size[0] % 2 == 1 else (
                                                                                        pooling_size[
                                                                                            0] - 1) // 2 + 1
            )
