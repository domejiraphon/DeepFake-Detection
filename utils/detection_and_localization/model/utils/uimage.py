...
def initialize_video_capture(source):
    ...
    try:
        ...
        _CAP = None
        print("Error on trying to read the following file as video: {}".format(
            source))
        print(
            "Please, check if the file exists, is an image and is not corrupted.")
...
def get_frame():
    ...
    if _CAP is None:
        ...
        try:
            ...
            print(
                "Error on getting a frame. Please, double-check if the video file is not corrupted."
            )
            print("Supported file format: MPEG-4 (*.mp4).")
            print(
                "Check whether working versions of ffmpeg or gstreamer is installed.")
...
def read(path_to_image, convert_to_grey_scale=False):
    ...
    if loaded_image is None:
        print(
            "Error on trying to read the following file as an image: {}".format(
                path_to_image
            )
        )
        print(
            "Please, check if the file exists, is an image and is not corrupted.")
        print(
            "Supported file formats: JPEG (*.jpeg and *.jpg) and Portable Network Graphics (*.png)."
        )
...
def draw_rectangle(
        image, initial_coordinates, final_coordinates, color=(0, 255, 0),
        thickness=2
):
    cv2.rectangle(image, initial_coordinates, final_coordinates, color,
                  thickness)
...
def draw_horizontal_bar(
    ...
    value_coordinates = (
        int(initial_coordinates[0] + ((x_length * val) / max)),
        final_coordinates[1],
    )

    cv2.rectangle(image, initial_coordinates, final_coordinates, color,
                  thickness)
    cv2.rectangle(image, initial_coordinates, value_coordinates, color,
                  cv2.FILLED)
...
def draw_graph(
    ...
    plt.plot(z, x, label=text_x, color=(np.array(color_x) / 255.0),
             linewidth=thickness)
    plt.plot(z, y, label=text_y, color=(np.array(color_y) / 255.0),
             linewidth=thickness)
    ...
    # Grid
    ax = plt.gca()
    ax.grid(color=(np.array(grid_color) / 255.0), linestyle="--")
    ax.spines["top"].set_color((np.array(grid_color) / 255.0))
    ...
    # Draw to image
    image[
    initial_coordinates[0]: initial_coordinates[0] + data.shape[0],
    initial_coordinates[1]: initial_coordinates[1] + data.shape[1],
    :,
    ] = data[:]


def draw_text(
        image, text, initial_coordinates, color=(0, 255, 0), scale=1,
        thickness=1
):
    cv2.putText(
        image,
        text,
        (int(initial_coordinates[0]), int(initial_coordinates[1])),
...
def draw_image(image, image_to_draw, initial_coordinates):
    image[
    int(initial_coordinates[0]): int(initial_coordinates[0])
                                 + image_to_draw.shape[0],
    int(initial_coordinates[1]): int(initial_coordinates[1])
                                 + image_to_draw.shape[1],
    :,
    ] = image_to_draw
...
def crop_rectangle(image, initial_coordinates, final_coordinates,
                   channels_last=True):
    if channels_last:
        return image[
               initial_coordinates[1]: final_coordinates[1],
               initial_coordinates[0]: final_coordinates[0],
               :,
               ]
    else:
        return image[
...
def superimpose(img_1, img_2, w_1=0.35, w_2=0.65, gamma=0):
    ...
    saliency_map = resize(
        img_1.cpu().detach().numpy(),
        output_size=(img_2.shape[1], img_2.shape[0])
    )
    saliency_map = cv2.applyColorMap(
        np.clip(saliency_map * 255, 0, 255).astype(np.uint8), cv2.COLORMAP_JET
    )
