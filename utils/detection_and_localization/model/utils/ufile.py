...
for i in range(_NUM_BRANCHES):
    _HEADER += ",Branch_{}_Emotion,Branch_{}_Arousal,Branch_{}_Valence".format(
        i, i, i)
_HEADER += "\n"
...
def write_to_file(fer, timestamp):
    ...
    try:
        ...
        if (fer is None) or (fer.list_emotion is None) or (
                fer.list_affect is None):
            for b in range(_NUM_BRANCHES + 1):
                line += ",None,None,None"
        else:
            line += ",{},{},{}".format(
                fer.list_emotion[-1], fer.list_affect[-1][1],
                fer.list_affect[-1][0]
            )
            for b in range(_NUM_BRANCHES):
                line += ",{},{},{}".format(
                    fer.list_emotion[b], fer.list_affect[b][1],
                    fer.list_affect[b][0]
                )
