...
class FERplus(Dataset):
    def __init__(
        ...
        self.fer_sets = {0: "FER2013Train/", 1: "FER2013Valid/",
                         2: "FER2013Test/"}
        ...
        print(
            "Size of the loaded set: {}".format(self.loaded_data[0].shape[0]))
    ...
    def __getitem__(self, idx):
        sample = {
            ...
            "emotion": self.loaded_data[1][idx],
        }
    ...
    def _load(self):
        ...
        path_folders_images = path.join(
            self.base_path_to_FER_plus, "Images", self.fer_sets[self.idx_set]
        )
        path_folders_labels = path.join(
            self.base_path_to_FER_plus, "Labels", self.fer_sets[self.idx_set]
        )
        ...
        for l in csv_label:
            ...
            if (emotion < self.num_labels) and (
                ...
                img = np.array(
                    uimage.read(path.join(path_folders_images, l[0])), np.uint8
                )
...
class AffectNetCategorical(Dataset):
    def __init__(
        ...
        if is_norm_by_mean_std:
            self.mean = [149.35457 / 255.0, 117.06477 / 255.0,
                         102.67609 / 255.0]
            self.std = [69.18084 / 255.0, 61.907074 / 255.0, 60.435623 / 255.0]
        else:
            self.mean = [0.0, 0.0, 0.0]
            self.std = [1.0, 1.0, 1.0]
    ...
    def _load(self):
        ...
        for f_af in folders_affect_net:
            ...
            for file_name_image_affect_net in images_affect_net:
                ...
                if (lbl >= 0) and (
                        counter_loaded_images_per_label[int(lbl)]
                        < self.max_loaded_images_per_label
                ):
                    img = np.array(
                        uimage.read(
                            path.join(
                                path_images_affect_net,
    ...
    def _load(self):
        ...
        for f_af in folders_affect_net:
            ...
            for file_name_image_affect_net in images_affect_net:
                ...
                if (quadrant >= 0) and (
                        counter_loaded_images_per_label[int(quadrant)]
                        < self.max_loaded_images_per_label
                ):
                    img = np.array(
                        uimage.read(
                    ...
                    labels_affect_net.append(
                        [np.float32(valence), np.float32(arousal)])
...
class CohnKanade(Dataset):
    def __init__(
            ...
            idx_test_fold,
            set_id,
            num_folds_supervised_training=None,
            idx_training_labeled_validation=None,
            transforms=None,
            base_path_to_dataset=None,
    ):
        """
        ...
        if (
                set_id == "training_labeled"
                or set_id == "training_labeled_validation"
                or set_id == "training_unlabeled"
                or set_id == "validation"
                or set_id == "test"
        ):
            self.set = set_id
        else:
            raise RuntimeError(
                "The 'set' variable must be 'training_labeled', 'training_unlabeled', 'validation' or 'test'."
            )
    ...
    def _load(self):
        ...
        supervised_training_fold_index = list(training_fold_index)[
                                         : self.num_folds_supervised_training
                                         ]

        if not (self.idx_training_labeled_validation is None):
            supervised_training_validation_fold_index = \
            supervised_training_fold_index[
                self.idx_training_labeled_validation
            ]
            training_fold_index.remove(
                supervised_training_validation_fold_index)

        training_fold_index = list(training_fold_index)

        supervised_training_fold_index = np.array(
            training_fold_index[: self.num_folds_supervised_training - 1],
            dtype=np.uint8,
        )
        unsupervised_training_fold_index = np.array(
            training_fold_index[self.num_folds_supervised_training - 1:],
            dtype=np.uint8,
        )
            ...
            np.array(
                data[
                    [
                        item
                        for sublist in
                        folds_data_index[supervised_training_fold_index]
                        for item in sublist
                    ]
                ]
            ),
        ...
        self.mean = np.mean(data / 255.0)
        self.std = np.std(data / 255.0)

        if self.set == "test":
            to_return = [
                np.array(data[folds_data_index[self.idx_test_fold]]),
                np.array(labels[folds_data_index[self.idx_test_fold]]),
            ]
        elif self.set == "validation":
            to_return = [
                np.array(data[folds_data_index[validation_fold_index]]),
                np.array(labels[folds_data_index[validation_fold_index]]),
            ]
        elif self.set == "training_labeled_validation":
            to_return = [
                np.array(
                    data[folds_data_index[
                        supervised_training_validation_fold_index]]
                ),
                np.array(
                    labels[folds_data_index[
                        supervised_training_validation_fold_index]]
                ),
            ]
            ...
            to_return = [
                np.array(
                    data[
                        [
                            item
                            for sublist in folds_data_index[
                            unsupervised_training_fold_index
                        ]
                            for item in sublist
                        ]
                    ]
...
def pre_process_affect_net(base_path_to_images, base_path_to_annotations,
    ...
    for line in range(image_id, annotation_file.shape[0]):
        try:
            ...
            img_full_path = path.join(
                base_path_to_images, annotation_folders[set_index],
                img_file_name
            )
            ...
            uimage.write(
                img,
                path.join(
                    base_path_to_images, destination_set_folders[set_index],
                    folder
                ),
                file_name,
            )
