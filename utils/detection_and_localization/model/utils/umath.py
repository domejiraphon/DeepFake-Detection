...
def plot(
    ...
    for d in range(len(data)):
        ...
        if not (upper_lower_data is None):
            plt.fill_between(
                data[d][0],
                np.array(upper_lower_data[d][0], dtype=float),
                np.array(upper_lower_data[d][1], dtype=float),
                where=np.array(upper_lower_data[d][0], dtype=float)
                      > np.array(upper_lower_data[d][1], dtype=float),
                alpha=0.5,
                interpolate=True,
            )
    ...
    if not (limits_axis_y is None):
        ...
        plt.yticks(
            np.arange(
                limits_axis_y[0], limits_axis_y[1] + limits_axis_y[2],
                limits_axis_y[2]
            )
        )
    ...
    if not (limits_axis_x is None):
        ...
        plt.xticks(
            np.arange(
                limits_axis_x[0], limits_axis_x[1] + limits_axis_x[2],
                limits_axis_x[2]
            )
        )
