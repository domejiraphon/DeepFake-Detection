...
class Base(nn.Module):
    ...
    def forward(self, x):
        ...
        x_shared_representations = self.pool(
            F.relu(self.bn2(self.conv2(x_shared_representations)))
        )
        x_shared_representations = F.relu(
            self.bn3(self.conv3(x_shared_representations))
        )
        x_shared_representations = self.pool(
            F.relu(self.bn4(self.conv4(x_shared_representations)))
        )
...
class ESR(nn.Module):
    ...
    def __init__(self, device):
        ...
        for i in range(1, len(self) + 1):
            ...
            self.convolutional_branches[-1].load_state_dict(
                torch.load(
                    path.join(
