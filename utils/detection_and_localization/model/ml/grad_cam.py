...
class GradCAM:
    ...
    def __init__(self, esr, device):
        ...
        for branch in esr.convolutional_branches:
            self._esr_branch_to_last_conv_layer.append(
                branch.forward_to_last_conv_layer
            )
    ...
    def __call__(self, x, i):
        ...
        output_activations = \
        self._esr_branch_from_last_conv_layer_to_emotion_output[i](
            feature_maps
        )
    ...
    def grad_cam(self, x, list_y):
        ...
        for i, y in enumerate(list_y):
            ...
            saliency_map = torch.zeros(feature_maps.size()[1:]).to(
                self._device)
