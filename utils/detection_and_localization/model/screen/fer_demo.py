...
class FERDemo:
    ...
    def __init__(
            self,
            window_name=_DEFAULT_WINDOW_NAME,
            screen_size=_DEFAULT_SCREEN_SIZE_ID,
            display_individual_classification=_DEFAULT_DISPLAY_INDIVIDUAL_CLASSIFICATION,
        ...
        self._container_center_position = np.array(
            [self._container_width // 2, self._container_height // 2],
            dtype=np.int
        )
        self._input_container = None
        self._output_container = None
        self._input_container_initial_position = np.array([0, 0], dtype=np.int)
        self._output_container_initial_position = np.array(
            [0, self._width // 2], dtype=np.int
        )
    ...
    def _draw_input_container(self, is_blank):
        self._input_container = self._get_container(
            0, 0, self._container_height, self._container_width
        )
        ...
        if is_blank:
            ...
            if h_ratio > w_ratio:
                if h < (
                        self._container_height * FERDemo._INPUT_IMAGE_SCALE_MIN):
                    f = (
                                self._container_height * FERDemo._INPUT_IMAGE_SCALE_MIN
                        ) / float(h)
                else:
                    f = (
                                self._container_height * FERDemo._INPUT_IMAGE_SCALE_MAX
                        ) / float(h)
            else:
                if w < (
                        self._container_height * FERDemo._INPUT_IMAGE_SCALE_MIN):
                    f = (
                                self._container_width * FERDemo._INPUT_IMAGE_SCALE_MIN
                        ) / float(w)
                else:
                    f = (
                                self._container_width * FERDemo._INPUT_IMAGE_SCALE_MAX
                        ) / float(w)
            ...
            self._input_container[x: (x + h), y: (y + w),
            :] = self._input_image
    ...
    def _generate_block(
            self, network_name, emotion, valence, arousal, face_image=None,
            x=0, y=0
    ):
        block = self._get_container(
            x, y, self._output_block_height, self._output_block_width
        )
        ...
        uimage.draw_horizontal_bar(
            ...
            if valence < 0.0
            else FERDemo._COLOUR_BGR_DARK_GREEN,
        )

        return block

    def _generate_block_ensemble(
            self, network_name, emotion, valence, arousal, face_image=None,
            x=0, y=0
    ):
        block = self._get_container(
            x, y, self._output_block_height_ensemble, self._output_block_width
        )
    ...
    def _draw_background(self):
        if (self._fer is None) or (self._fer.input_image is None):
            self._background = (
                    np.ones((self._height, self._width, 3), dtype=np.uint8)
                    * FERDemo._COLOUR_G_DARK_GREY
            )
        else:
            # Resize
            self._background = uimage.resize(
                ...
                f=np.maximum(
                    np.maximum(
                        self._fer.input_image.shape[0] / self._height,
                        self._fer.input_image.shape[1] / self._width,
                    ),
                    np.maximum(
                        self._height / self._fer.input_image.shape[0],
            ...
            self._background = np.clip(
                (gamma * self._background) + mean, 0, 255
            ).astype(np.uint8)
    ...
    def is_running(self):
        return (cv2.waitKey(1) != 27) and (
                cv2.getWindowProperty(self._window_name,
