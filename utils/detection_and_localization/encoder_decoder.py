...
def get_argparser():
    ...
    # Datset Options
    parser.add_argument(
        "--data_root", type=str, default="./datasets/data",
...
def get_dataset(opts):
    ...
    if opts.dataset == "cityscapes":
        ...
        val_dst = Cityscapes(root=opts.data_root, split="val",
                             transform=val_transform)
        ...
        for i, (images, labels) in tqdm(enumerate(loader)):
            ...
            if opts.save_val_results:
                for i in range(len(images)):
                    ...
                    image = (denorm(image) * 255).transpose(1, 2, 0).astype(
                        np.uint8)
                    target = loader.dataset.decode_target(target).astype(
                        np.uint8)
        ...
        for (images, labels) in train_loader:
            ...
            if (cur_itrs) % opts.val_interval == 0:
                ...
                if vis is not None:  # visualize validation score and samples
                    vis.vis_scalar(
                        "[Val] Overall Acc", cur_itrs, val_score["Overall Acc"]
                    )
                    ...
                    for k, (img, target, lbl) in enumerate(ret_samples):
                        ...
                        target = (
                            train_dst.decode_target(target)
                                .transpose(2, 0, 1)
                                .astype(np.uint8)
                        )
                        lbl = (
                            train_dst.decode_target(lbl)
                                .transpose(2, 0, 1)
                                .astype(np.uint8)
                        )
