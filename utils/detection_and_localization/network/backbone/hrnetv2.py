...
class StageModule(nn.Module):
    def __init__(self, stage, output_branches, c):
        ...
        self.number_of_branches = (
            stage
        # number of branches is equivalent to the stage configuration.
        )
...
class HRNet(nn.Module):
    def __init__(self, c=48, num_blocks=[1, 4, 3], num_classes=1000):
            ...
            [
                ...
                nn.Sequential(
                    nn.Sequential(
                        ...
                        nn.Conv2d(
                            256, c * 2, kernel_size=3, stride=2, padding=1,
                            bias=False
                        ),
                        nn.BatchNorm2d(
                            c * 2, eps=1e-05, affine=True,
                            track_running_stats=True
                        ),
                        nn.ReLU(inplace=True),
                    )
        ...
        self.bn_classifier = nn.Sequential(
            nn.Conv2d(out_channels, out_channels // 4, kernel_size=1,
                      bias=False),
            nn.BatchNorm2d(
                out_channels // 4, eps=1e-05, affine=True,
                track_running_stats=True
            ),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        x1 = F.interpolate(
            x[1], size=(output_h, output_w), mode="bilinear",
            align_corners=False
        )
        x2 = F.interpolate(
            x[2], size=(output_h, output_w), mode="bilinear",
            align_corners=False
        )
        x3 = F.interpolate(
            x[3], size=(output_h, output_w), mode="bilinear",
            align_corners=False
        )
...
def _hrnet(arch, channels, num_blocks, pretrained, progress, **kwargs):
    ...
    if pretrained:
        logger.info("Loading pretrained backbone HRNetV2 model .....")
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("--- Running file as MAIN ---")
        logger.info(
            f"Backbone HRNET Pretrained weights as __main__ at: {CKPT_PATH}")
    except:
        logger.info(
            "No backbone checkpoint found for HRNetv2, please set pretrained=False when calling model"
        )
